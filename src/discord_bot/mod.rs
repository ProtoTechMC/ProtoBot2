...
impl EventHandler for Handler {
    ...
    async fn message(&self, ctx: Context, new_message: Message) {
        ...
        tokio::runtime::Handle::current().spawn(async move {
            ...
            let message_handling = {
                if GuildStorage::get(guild_id).await.octal_counter_state.octal_counter_channel == Some(new_message.channel_id) {
    ...
    async fn message_update(
        ...
        } else if GuildStorage::get(match event.guild_id { Some(i) => i, None => return }).await.octal_counter_state.octal_counter_channel == Some(event.channel_id) {
            let Some(author) = event.author else { return; };
            tokio::runtime::Handle::current().spawn(async move {
